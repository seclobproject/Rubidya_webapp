...
const Search = () => {
  ...
  return (
    <div className="w-full h-screen pb-4 lg:pt-24  lg:w-[685px] px-4 py-6 flex justify-center items-center font-poppins">
      <div className=" w-full h-full px-5  lg:px-[24px]  bg-white rounded-t-md flex flex-col">
        ...
        <div
          className={`flex flex-col h-full overflow-y-scroll items-center ${
        ...
        >
          {loading ? (
            ...
            results &&
            results.slice(0, 50).map((user, index) => {
              return (
                <Link to={`/user/${user._id}`} className="w-full" key={index}>
                  <SingleSearchCard user={user}  />
                </Link>
              );
            })
          )}

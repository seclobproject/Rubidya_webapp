...
import { removeFromUserSuggestions, setFollowing } from "../config/rubidyaSlice";
...
const SuggestUserCard = ({user}) => {
  ...
  return (
    <div className="w-[125px] h-fit py-[11px] bg-[#FFFFFF]   rounded-lg flex flex-col    justify-center items-center " style={{boxShadow: '0px 2px 5px rgba(0, 0, 0, 0.1)',
    borderRadius: '4px',
    padding: '1rem',}}>
      <div className="relative mb-[8px]">
        <img src={user?.profilePic?.filePath || pic} alt="" className="w-16 h-16 rounded-full" />
        <img src={RIcon} alt="" className="absolute -right-1 -bottom-1 w-6 h-6" />
      </div>
      <div className="text-[#1E3167] text-xs mb-[6px]">{user?.firstName.split(" ")[0]}</div>
      <div className="">
        {!status &&<button className="text-white bg-[#45537A] rounded-md py-1 px-6 text-xxs2" onClick={handleFollow} disabled={Loading}>
